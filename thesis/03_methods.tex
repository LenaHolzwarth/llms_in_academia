...
\emph{Pubmed Central}
\\
%comment: check the contents of pmc, is there also biorxiv? are papers listed double, once as preprint and once as published version, add information about most important journals in the set, 
...
%comment: beta hat is not technically an estimate but a lower bound, but we can call it an estimate after optimization because we approximate p_llm = 1 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{../results/plots/misc/exess_words_18_24.png}
    \caption{}
...
This turned out to be non-feasible, because the length of the full papers lead to finding a huge number of excess words, which would all have to be manually annotated to filter them for style words.
...
\begin{figure}[htbp]
    ...
    %comment: more detail in caption
...
%comment: all of it should be in appendix