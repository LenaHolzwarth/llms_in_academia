...
\emph{Pubmed Central}
\\
%comment: follow up on inquiry to nih
Pubmed Central (PMC) is a collection of open-access biomedical literature curated by the U.S. National Institutes of Health's National (NIH) Library of Medicine \cite{pmc}. 
...
The baseline was downloaded via FTP, which is since deprecated\footnote{https://ftp.ncbi.nlm.nih.gov/pub/pmc/deprecated/oa\_bulk/}.
\\
...
For this purpose, all analyses detailed in the following subsections were performed on the abstract, introduction, methods, results and discussion of the paper, as well as the full paper, which refers to the four sections from the body without the abstract.
With the finished dataset, we can turn to the methods used to estimate the proportion of LLM-generated text in it.
...
%comment:more detail? this under-estimation is too large to be an estimate?
...
\begin{figure}[htbp]
    ...
    \caption{Illustration of the frequency gap as a lower bound for LLM-use and why it (strongly) underestimates true LLM-use. In this example, we assume an underlying distribution that is shown in the top bar. Humans are assumed to use a word in 60\% of texts, so $\ph=0.6$. This proportion is shown by the blue bar. We further assume that LLMs use a word in 80\% of texts ($\pl=0.8$), which is indicated by the green bar. LLM-use $\beta$ is assumed to be 0.5, meaning that half of the texts in the sample are LLM-assisted. When sampling from this distribution in the second row, we observe that in total, 70\% of texts use this word. This observation is the word frequency $q$ that we measure for PMC. In this example, we know that the 70\% of overall usage can be explained by $\ph\cdot(1-\beta)= 60\% \cdot 0.5 = 30\%$ of humans using the word, plus $\pl\cdot\beta= 80\% \cdot 0.5 = 40\%$ of LLMs using the word. However, when trying to estimate LLM-use, we only have access to $\ph$ (indirectly, via regression). Since we don't know $\beta$, the most conservative estimate is $\beta=0$, i.e. no texts are written with the help of LLMs. This means that we expect $\ph\cdot(1-\beta)= 60\% \cdot 1 = 60\%$ of papers to include a term if all texts were written by humans, meaning the remaining $\Delta = q - \ph = 10\%$ of word occurrences must have been written by an LLM and therefore the rate of LLM-use must at least be 10\%. This method of lower-bounding the LLM-use thus underestimates the true proportion of LLM-assisted text by 40 percentage points.} 
...
To protect against these fluctuations, we replace individual words with word lists.
...
For Kobak et al., who use the frequency gap $\Delta$ as a lower bound for LLM-use, this is suboptimal because the closer $\hph$ is to 1, the smaller the potential gap.
When using $\beta_{lb}$ as the metric, the highest lower bound would be achieved by getting $p_{LLM}$ as close to 1 as possible, which would in turn move $\beta_{lb}$ closer to the true value $\beta$.
While $p_{LLM}$ isn't observable, it can be manipulated by increasing the number of words in the excess list.
However, this includes the caveat that $\hph$ must remain smaller than 1, otherwise, Equation \ref{eq:beta_hat} yields division by zero.
...
It can be read as the lower bound on LLM-use for the optimal word list.
...
We conceptualize $q$ as the MLE for the binomial probability of an excess word to appear in a paper.
...
%comment: is assumption of independence valid?
...
We simulated data for $n=100,000$ texts, approximating the number of papers per year in our sample.
In our simulation, there were $m=500$ words, all of which were treated as excess words.
We created two matrices, $M_{pre}$ and $M_{post}$, each with dimensions $(n,m)$, corresponding to data before and after the release of ChatGPT, respectively.
...
Then, $n$ binomial samples were drawn with $\ph(w)$ as probability of success for each word.
For $M_{post}$, a fraction of $\beta$ texts were selected as being LLM-generated.
As our assumptions require that $\ph\le\pl$ (because we define excess words to have a frequency increase after the release of ChatGPT), the probabilities for words to occur in LLM-assisted text were sampled as $\pl(w)=\ph(w)(1+\delta)$, with $\delta \sim U(0.5,5)$.
In cases where $\ph(w)(1+\delta)>1$, $\delta$ was newly sampled until the resulting $\pl(w)$ was in the range $[0,1]$.
In the sampling process $n\cdot \beta$ binomial samples were drawn with probability $\pl(w)$, and $n\cdot (1-\beta)$ were drawn with probability $\ph(w)$.
\\
For the following analyses, the word frequencies in $M_{pre}$ were treated as analogous to $\hph$, and those in $M_{post}$ as the observed frequencies $q$.
...
%comment: Figure here shows the different section lengths
...
This was implemented by cropping each section sample to a length of 255 words, which corresponds to the median length of the abstracts\footnote{This was computed with the previous baseline, for the current baseline the median abstract length is 248}.
The cropped part was placed randomly within the section by sampling a starting position ranging between the first and 255th-to-last words in the text.
...
Previous studies typically reported higher usage estimates for research areas related to computer science \cite{Liang2024_paper, Liang2025}.
...
The list of countries and their respective paper counts can be found in the Appendix. %comment: this is missing, delete the sentence if don't have time to add
\\
...
\\
Papers were sorted into different research areas based on the titles of journals in which they were published in a procedure based on González-Márquez et al. \cite{gonzalez2024landscape}.
...
%comment: write explanation why the results are in appendix
