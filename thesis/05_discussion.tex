...
Science relies on written communication, the production of which is the core competency of these models.
...
Crucially, there is no ground-truth value as a goal for estimation, because the quantity to be estimated, the prevalence of LLM-use specifically for writing scientific papers, is unknown.
...
While this gap is the highest difference of results (at least those observed within a similar time span), it is indicative of an overall trend: Surveys on average seem to report higher estimates than indirect estimation methods who try to infer LLM-use from the text of published papers.
This might seem counter-intuitive, because amid concerns of using LLMs for scientific fraud, %comment: citation
researchers are likely more incentivized to lie about or downplay their LLM-use, especially when the survey is done by publishing houses such as Wiley, Elsevier or Nature \cite{Elsevier2024,Kwon2025,VanNoorden2023,Nordling2023,Wiley2026,Wiley2025}.
This might be somewhat mediated by the anonymity of the surveys.
On the other hand, researchers are probably unlikely to exaggerate their use of LLMs.
Also, some of the use-cases are highly specific and thus offer no reliable estimate of overall LLM-use in science.
For example, 28\% of respondents of a Nature study \cite{Kwon2025} say they use LLMs to edit their paper, while 8\% say they use them to write the first draft, but since response overlap is not reported, the total percentage of researchers in the sample using LLMs for either of these tasks could be as high as 36\% if there is no overlap of use-cases, and this doesn't even take into account that people might use LLMs in their paper-writing process for other purposes, such as re-writing paragraphs to adapt the tone or style.
Given all of this, we can think of the survey results as a lower bound on LLM-use in academia, which points to the unreliability of indirect estimation methods, which frequently yield lower estimates than the surveys. %comment: really not sure about this 
...
\\
The estimation methods relying on word frequency (Gray \cite{Gray2025,Gray2026}, Kobak et al. \cite{Kobak2025} and Kousha and Thelwall\cite{Kousha2025}) differences all yield similar estimates that are lower than those reported in this thesis.
...
While Geng and Trotta \cite{Geng2024} and Liang et al. \cite{Liang2024_paper} both use word frequency modeling on computer science papers from arXiv only a month apart, the estimate reported by Geng and Trotta, 35\%, is nearly twice as high as that of Liang et al. with 18\%.
...
Unless a wide variety of prompts and models is used in the creation, the training set will likely misrepresent the actual distribution of LLM-modulated text, thus biasing the estimates.
Geng and Trotta are also among the few studies with indirect estimates that are higher than those reported in this thesis, which is plausible considering the different dataset they used: while PMC also contains some preprints, %comment: verify
the majority is published work, which means that there is a longer gap between writing and publishing, so arXiv is likely to have a higher proportion of recent research, which, assuming increasing adoption of LLMs into the academic writing process, will translate to higher usage rates.
Also, they limited their sample to papers from computer science, which is the field most likely to quickly adapt to new AI technology. %citation?
\\
The estimates from commercial LLM detectors who evaluate LLM-use on a case-by-case basis are harder to evaluate, because it is unclear how the models are assembled and trained.
...
That being said, since the publications of these studies there have been considerable improvements in LLM-detectors, for example by training models to not only be able to detect fully machine-generated text, but also text revised or edited in parts by LLMs \cite{Chen2025_revise}, which is much closer to the way LLMs are used in academia.
...
To investigate this, the analysis could be expanded to include higher-order n-grams \cite{Gall2021}.
...
In reality, most researchers do not disclose their LLM-use \cite{Kousha2024,Kwon2025}, and an ever-increasing trend of fake citations implies that scientists either neglect the verification of LLM output or use them to perform scientific fraud at a previously unknown scale \cite{Kendall2024, Walters2023, lancet study}.
These developments give bad actors an excuse to undermine scientific institutions and could reduce trust in the process of science.
As Andrew Gray put it, any appeal to empirical evidence can now be countered with ``Oh, you can’t trust that anyway, don’t you know it’s all AI rubbish now?'' \cite{Gray2026}.
