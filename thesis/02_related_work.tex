...
For example, Gray \cite{Gray2025} computed the relative yearly change of word frequency in papers from the Dimensions database, both for single words and word groups of up to twelve terms.
The terms were selected from a list of LLM marker words identified by Liang et al. \cite{Liang2024_review}.
...
By computing the MLE for this corpus, the researchers were able to retrieve the correct proportion.
\\
...
However, this validation is strongly dependent upon the exact model and prompt that were used to create the LLM corpus, and the estimated distribution based on this corpus might differ drastically from the true LLM distribution, which is likely to have been created from a multitude of prompts and models.
